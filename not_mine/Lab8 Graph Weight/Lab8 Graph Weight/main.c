...
void addEdge(int from, int to, int weight, Vertex** vertexListHead) {
    ...
    while (ptrTo != NULL && ptrTo->key != to) ptrTo = ptrTo->next;
    while (ptrFrom != NULL && ptrFrom->key != from) ptrFrom = ptrFrom->next;

    if (ptrTo == NULL) ptrTo = addVertex(to, vertexListHead);
    if (ptrFrom == NULL) ptrFrom = addVertex(from, vertexListHead);
    ...
    newEdge->next = NULL;
    if (ptrFrom->adjListHead == NULL) ptrFrom->adjListHead = newEdge;
    else {
