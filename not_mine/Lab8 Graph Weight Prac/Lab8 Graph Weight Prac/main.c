...
void addEdge(int start, int end, int weight, Vertex **vertexListHead)
{
    ...
    while (ptrStart != NULL && ptrStart->key != start)
        ptrStart = ptrStart->next;
    while (ptrEnd != NULL && ptrEnd->key != end)
        ptrEnd = ptrEnd->next;

    if (ptrStart == NULL)
        ptrStart = addVertex(start, vertexListHead);
    if (ptrEnd == NULL)
        ptrEnd = addVertex(end, vertexListHead);
    ...
    newEdge->next = NULL;
    if (ptrStart->adjListHead == NULL)
        ptrStart->adjListHead = newEdge;
    else
    {
